...
VALUES
  ('DOCUMENTOS', 'DOC_LICENCIA_CONDUCCION', 'Licencia de conducción', 1, TRUE),
  ('DOCUMENTOS', 'DOC_SOAT', 'SOAT', 2, TRUE),
  ('DOCUMENTOS', 'DOC_REVISION_TECNOMECANICA', 'Revisión tecnomecánica', 3, TRUE),
  ...
  ('CABINA', 'CAB_DIRECCION_SUSPENSION', 'Dirección/suspensión (terminales)', 5, TRUE),
  ('CABINA', 'CAB_CINTURON_SEGURIDAD', 'Cinturón de seguridad', 6, TRUE),
  ...
  ('CABINA', 'CAB_BOTIQUIN', 'Botiquín de primeros auxilios', 10, TRUE),
  ...
  ('ESTADO_MECANICO', 'MEC_CONTROL_FUGAS', 'Control de fugas hidráulicas', 1, TRUE),
  ('ESTADO_MECANICO', 'MEC_PASADORES_SUSPENSION', 'Pasadores, suspensión', 2, TRUE),
  ...
  ('ESTADO_MECANICO', 'MEC_CADENA_CARDAN', 'Cadena cardán', 5, TRUE),
  ...
  ('SEGURIDAD_SOCIAL', 'SOC_PENSION', 'PENSIÓN', 3, TRUE)
ON CONFLICT (codigo) DO NOTHING;

INSERT INTO menu_items (tenant_id, key, module, label, route, visible, metadata)
SELECT t.id, 'INSPECCION_DIARIA', 'operaciones', 'Inspección diaria', '/{tenant}/inspeccion-diaria', TRUE, '{}'::jsonb
FROM tenants t
WHERE NOT EXISTS (
  SELECT 1 FROM menu_items mi
  WHERE mi.tenant_id = t.id AND mi.key = 'INSPECCION_DIARIA' AND mi.deleted_at IS NULL
);

INSERT INTO role_menu_permissions (tenant_id, role_id, menu_item_id, access_level)
SELECT t.id, r.id, mi.id, level.access_level
FROM tenants t
INNER JOIN roles r ON r.tenant_id = t.id AND upper(r.nombre) IN ('ADMIN','SUPER_ADMIN','USER')
INNER JOIN menu_items mi ON mi.tenant_id = t.id AND mi.key = 'INSPECCION_DIARIA' AND mi.deleted_at IS NULL
CROSS JOIN LATERAL (
  VALUES ('READ'::text), ('WRITE'::text)
) AS level(access_level)
WHERE NOT EXISTS (
  SELECT 1
  ...
  WHERE rmp.tenant_id = t.id
    ...
    AND rmp.access_level = level.access_level
);
